...
import me.superckl.api.superscript.script.ScriptParser.ScriptLine;
...
public class ScriptHandler {
	...
	public ScriptHandler(final List<ScriptLine> scriptLines) {
		...
		for(final Entry<String, Class<? extends ScriptObject>> entry:ScriptHandler.staticObjects.entrySet())
			try {
				this.objects.put(entry.getKey(), entry.getValue().getConstructor().newInstance());
	...
	public void parse() throws Exception{
		...
		while(it.hasNext()){
			...
			final String s = line.line().split("#", 2)[0].trim(); //Cuts out comments at the end of a line
			try {
				if(s.contains("=")){
					final Map<String, Object> map = ScriptParser.parseAssignment(s, line.context(), this);
					if(map == null)
						continue;
					for(final Entry<String, Object> entry:map.entrySet())
						if(entry.getValue() instanceof String)
							...
							((ScriptObject) entry.getValue()).setContext(line.context());
							this.objects.put(entry.getKey(), (ScriptObject) entry.getValue());
						}
				}else if(s.contains(".")){
					final String[] split = s.split("[.]", 2);
					if(split.length != 2) {
						APIInfo.log.error("Failed to handle a script line! "+s+" @ "+line.context()+". Reason: Found operator '.' in invalid context.");
						continue;
					}
					if(!this.objects.containsKey(split[0])) {
						APIInfo.log.error("Failed to handle a script line! "+s+" @ "+line.context()+". Reason: Object not found: "+split[0]);
						continue;
					}
					final ScriptObject obj = this.objects.get(split[0]);
					obj.handleCall(split[1], line.context(), this);
				}
			} catch (final Exception e) {
				APIInfo.log.error("Failed to handle a script line! "+s+" @ "+line.context()+". Reason: "+e.getMessage());
				APIInfo.log.debug("Full stacktrace of error provided below for bug reports.", e);
			}
		}
	}
